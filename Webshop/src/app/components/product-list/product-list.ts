...
export class ProductList implements OnInit {
  ...
  toggleGenre(id: number) {
    if (this.selectedGenreId === id) {
      this.selectedGenreId = null; // kikapcsolás
    } else {
      this.selectedGenreId = id; // kiválasztás
    }
  }
  ...
  getBookPage() {
    this.bookService.getBooksPage(this.pageNumber, this.sortType, this.isAsc).subscribe({
      next: (response) => {
        ...
        this.availablePages = Array(pageNumber)
          .fill(1)
          .map((x, i) => i + 1);
      },
  ...
  changePageWithArrows(isForward: boolean) {
    if (
      (this.pageNumber - 1 >= 0 && !isForward) ||
      (this.pageNumber + 1 < this.availablePages.length && isForward)
    ) {
